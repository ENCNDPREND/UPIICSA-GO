<script>
    ...
    const buscarMaterias = async () => {
      try {
        const response = await fetch(`https://upiicsa-go-back.onrender.com/buscar-materias?q=${query}&email=${$Usuario.email}`);
    ...
    const agregarMateria = async (idMateria) => {
      try {
        const response = await fetch('https://upiicsa-go-back.onrender.com/agregar-materia-alumno', {
          method: 'POST',
          headers: {
